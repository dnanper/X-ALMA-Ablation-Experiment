...
def load_model_specific(base_model_path, adapter_model_path, model_args, data_args, tokenizer, logger):
    """
    Load model with specific base model and adapter/module combination.
    This is useful for ablation studies to test different base models with X-ALMA modules.
    
    Args:
        base_model_path: HuggingFace path to base model (e.g., "meta-llama/Llama-2-13b-hf" or "haoranxu/ALMA-13B-Pretrain")
        adapter_model_path: HuggingFace path to adapter/module (e.g., "haoranxu/X-ALMA-13B-Group5")
    ...
    Returns:
        model: Loaded model with adapter applied
    
    Example:
        # Test contribution of pretrain: use unpretrained base with X-ALMA module
        model = load_model_specific(
            base_model_path="meta-llama/Llama-2-13b-hf",  # Unpretrained
            adapter_model_path="haoranxu/X-ALMA-13B-Group5",
    ...
    """
    logger.info(f"Loading specific base model from: {base_model_path}")
    logger.info(f"Loading adapter/module from: {adapter_model_path}")
    ...
    if model_args.multi_gpu_one_model:
        base_model = AutoModelLoad.from_pretrained(
            base_model_path,
            device_map="auto",
    ...
    # Load adapter/module on top of base model
    ...
    logger.info(f"  - Max length: {model.generation_config.max_length}")
